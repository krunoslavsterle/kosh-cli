...
export default function App() {
  ...
  const fetchApiStatus = async () => {
    ...
    setApiError(null)
    try {
      // Query API via domain or direct localhost
  ...
  return (
    <div className="container">
      <header>
        <span className="badge">⚛️ React Frontend App</span>
        <h1>Kosh CLI Demo — React Edition</h1>
        <p className="subtitle">Microservice Orchestration & Real-time Log Streaming</p>
      </header>

      <main>
        {/* Framework Navigation Bar */}
        <div style={{ display: 'flex', gap: '16px', marginBottom: '32px', justifyContent: 'center' }}>
          <div style={{ padding: '12px 24px', background: 'rgba(56, 189, 248, 0.2)', border: '1px solid #38bdf8', borderRadius: '12px', color: '#38bdf8', fontWeight: 600 }}>
            🔒 https://kosh-demo.react.localhost
          </div>
          <a href="https://kosh-demo.angular.localhost" style={{ padding: '12px 24px', background: 'rgba(239, 68, 68, 0.1)', border: '1px solid rgba(239, 68, 68, 0.3)', borderRadius: '12px', color: '#f87171', fontWeight: 600, textDecoration: 'none', transition: 'all 0.2s' }}>
            🅰️ Switch to https://kosh-demo.angular.localhost →
          </a>
        </div>
        ...
        <div className="card" style={{ marginBottom: '24px' }}>
          ...
          {loading ? (
            <p style={{ color: '#94a3b8' }}>Querying https://kosh-demo.api.localhost/api/status...</p>
          ) : apiError ? (
            ...
            <div>
              ...
              <pre style={{ background: 'rgba(0,0,0,0.5)', padding: '12px', borderRadius: '8px', fontSize: '0.8rem', color: '#38bdf8', overflowX: 'auto' }}>
          ...
          <div style={{ marginTop: '16px', display: 'flex', gap: '12px' }}>
            <button onClick={fetchApiStatus} style={{ padding: '8px 16px', background: '#38bdf8', color: '#0f172a', border: 'none', borderRadius: '8px', fontWeight: 600, cursor: 'pointer' }}>
              🔄 Send Request to API
            </button>
        ...
        <div className="cli-usage">
          ...
          <p style={{ fontSize: '0.85rem', color: '#94a3b8' }}>
            Inside Kosh TUI: type <code>:v api</code> for API logs, <code>:v frontend-react</code> for React logs!
          </p>
        </div>
      </main>
    </div>
